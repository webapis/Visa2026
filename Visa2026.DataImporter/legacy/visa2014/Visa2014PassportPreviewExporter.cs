...
internal static class Visa2014PassportPreviewExporter
{
    public static Visa2014PersonImportBatch PrepareImportBatch(
        ...
        int? maxRows,
        bool verbose) =>
        Visa2014PassportTransform.PrepareImportBatch(connectionString, lookupTranslationPaths, maxRows, verbose);
        ...
        string? legacySourceId = null)
    {
        ...
        var metaRows = new List<IReadOnlyDictionary<string, object?>>
        {
            ...
            Row("_key", "fieldMap", "legacy/visa2014/field-maps/Passport.yaml"),
        };
        ...
        [
            new Visa2014Worksheet { Name = "Passport", Columns = Visa2014PassportTransform.PassportMainColumnOrder, Rows = batch.ImportRows },
            new Visa2014Worksheet { Name = "_Skipped", Columns = Visa2014PersonTransform.InferColumns(batch.Skipped.ToList(), Visa2014PassportTransform.PassportMainColumnOrder), Rows = batch.Skipped.ToList() },
            new Visa2014Worksheet { Name = "_UnmappedLookups", Columns = ["catalog", "legacyValue", "reason"], Rows = batch.UnmappedLookups.ToList() },
            new Visa2014Worksheet { Name = "_DedupeSummary", Columns = Visa2014PersonTransform.InferColumns(batch.DedupeSummary.ToList(), ["_dedupeGroupId", "key", "normalizedValue", "memberCount", "canonical_legacyRowId", "canonicalRule"]), Rows = batch.DedupeSummary.ToList() },
