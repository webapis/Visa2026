...
internal static class Visa2014HospitalPreviewExporter
{
        ...
        string? legacySourceId = null)
    {
        ...
        var batch = Visa2014HospitalTransform.PrepareImportBatch(connectionString, lookupTranslationPaths, maxRows, verbose);

        var metaRows = new List<IReadOnlyDictionary<string, object?>>
        {
            Row("_key", "exportedAt", DateTime.UtcNow.ToString("O")),
            Row("_key", "entity", "Hospital"),
            ...
            Row("_key", "source", "VISA2015 Address (DocumentOfAddress TypeOfDocument=myhmanhana, hospital classifier)"),
            Row("_key", "normalizer", "Visa2014AddressLineNormalizer.NormalizeHospitalCatalogName"),
        };
        ...
        var dedupeColumns = Visa2014PersonTransform.InferColumns(
            ...
            Visa2014HospitalTransform.HospitalMainColumnOrder);

        var writtenPath = Visa2014MinimalXlsxWriter.WriteWorkbook(outputPath,
        [
            new Visa2014Worksheet { Name = "Hospital", Columns = Visa2014HospitalTransform.HospitalMainColumnOrder, Rows = batch.ImportRows },
