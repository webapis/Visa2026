...
internal static class Visa2014HotelPreviewExporter
{
        ...
        string? legacySourceId = null)
    {
        ...
        var batch = Visa2014HotelTransform.PrepareImportBatch(connectionString, lookupTranslationPaths, maxRows, verbose);

        var metaRows = new List<IReadOnlyDictionary<string, object?>>
        {
            Row("_key", "exportedAt", DateTime.UtcNow.ToString("O")),
            Row("_key", "entity", "Hotel"),
            ...
            Row("_key", "source", "VISA2015 Address (myhmanhana + Lojman lines with myhmanhan*)"),
            Row("_key", "normalizer", "Visa2014AddressLineNormalizer.NormalizeHotelCatalogName"),
        };
        ...
        var dedupeColumns = Visa2014PersonTransform.InferColumns(
            ...
            Visa2014HotelTransform.HotelMainColumnOrder);

        var writtenPath = Visa2014MinimalXlsxWriter.WriteWorkbook(outputPath,
        [
            new Visa2014Worksheet { Name = "Hotel", Columns = Visa2014HotelTransform.HotelMainColumnOrder, Rows = batch.ImportRows },
