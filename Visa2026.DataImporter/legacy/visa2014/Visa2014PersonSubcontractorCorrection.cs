...
internal sealed class Visa2014PersonSubcontractorCorrectionResult
{
    public int PersonsInScope { get; init; }
...
internal static class Visa2014PersonSubcontractorCorrection
{
    public static async Task<int> RunCommandAsync(IReadOnlyList<string> args, bool verbose)
    {
        ...
        Console.WriteLine("=== VISA2014 Person Subcontractor correction");
        ...
        try
        {
            ...
            Console.WriteLine($"INF Persons in scope: {result.PersonsInScope}");
            Console.WriteLine($"INF Subcontractor updated: {result.Updated}");
    ...
    internal static Task<Visa2014PersonSubcontractorCorrectionResult> RunAsync(
        ...
        bool verbose)
    {
        ...
        {
            return Task.FromResult(new Visa2014PersonSubcontractorCorrectionResult
            {
                Errors = ["Person id-map is empty — import Person first or pass --person-id-map."],
            });
        }

        var catalogs = Visa2014LookupTranslator.Load(lookupTranslationPaths);
        var legacyNames = LoadLegacySubcontractorNames(legacyConnectionString, personIdMap.Keys, verbose);
        var employeeNameByLegacyOid = legacyNames
            .Where(kv => kv.Value.IsEmployee && !string.IsNullOrWhiteSpace(kv.Value.ResolvedName))
            .ToDictionary(kv => kv.Key, kv => kv.Value.ResolvedName!);

        int updated = 0, unchanged = 0, unresolved = 0;
        ...
        {
            if (!legacyNames.TryGetValue(legacyOid, out var legacy))
                continue;

            var subcontractorName = legacy.ResolvedName;
            if (!legacy.IsEmployee
                && string.IsNullOrWhiteSpace(subcontractorName)
                && legacy.LegacyEmployeeOid.HasValue)
            {
                employeeNameByLegacyOid.TryGetValue(legacy.LegacyEmployeeOid.Value, out subcontractorName);
            }

            if (!Visa2014LookupTranslator.TryTranslate(
                    catalogs, "Subcontractor", subcontractorName, out var translatedName, out _))
            {
                translatedName = subcontractorName;
            }

            var subcontractorId = resolver.ResolveSubcontractor(translatedName);
            if (!subcontractorId.HasValue)
            {
                unresolved++;
                if (verbose)
                    Console.WriteLine($"WRN {legacyOid}: unresolved subcontractor '{subcontractorName}'");
                continue;
            }
            ...
            if (person.Subcontractor?.ID == subcontractorId)
            {
                unchanged++;
                continue;
            }

            var subcontractor = objectSpace.GetObjectByKey<Bo.Subcontractor>(subcontractorId);
            if (subcontractor == null)
            {
                errors.Add($"{legacyOid}: Subcontractor {subcontractorId} not found");
                continue;
            }
        ...
        return Task.FromResult(new Visa2014PersonSubcontractorCorrectionResult
        {
            PersonsInScope = personIdMap.Count,
            Updated = updated,
            Unchanged = unchanged,
    ...
    private sealed record LegacySubcontractorRow(bool IsEmployee, Guid? LegacyEmployeeOid, string? ResolvedName);

    private static Dictionary<Guid, LegacySubcontractorRow> LoadLegacySubcontractorNames(
        string legacyConnectionString,
        ...
        bool verbose)
    {
        ...
        if (oidList.Count == 0)
            return new Dictionary<Guid, LegacySubcontractorRow>();
        ...
        if (verbose)
            Console.WriteLine($"INF Loading legacy subcontractor names for {oidList.Count} person(s)...");

        var rows = Visa2014SqlCmdReader.Query(legacyConnectionString, sql, verbose: false);
        var map = new Dictionary<Guid, LegacySubcontractorRow>();
        foreach (var row in rows)
        {
            if (!Guid.TryParse(row.GetValueOrDefault("Oid"), out var legacyOid))
                continue;

            var isEmployee = row.GetValueOrDefault("IsEmployee") == "1";
            Guid? legacyEmployeeOid = Guid.TryParse(row.GetValueOrDefault("LegacyEmployeeOid"), out var emp) ? emp : null;
            var name = row.GetValueOrDefault("LegacySubcontractorName");
            map[legacyOid] = new LegacySubcontractorRow(isEmployee, legacyEmployeeOid, name);
        }
