...
public class Visa2014ApplicationProfileInstanceProgressCompletionIndexTests
{
    [Fact]
    public void BuildEvidence_PrefersInvitationWhenLaterDate()
    {
        var evidence = Visa2014ApplicationProfileInstanceProgressCompletionIndex.BuildEvidence(
    ...
    {
        var partial = Visa2014ApplicationProfileInstanceProgressCompletionIndex.BuildVisaExtensionEvidence(
            new Dictionary<string, string?>(StringComparer.Ordinal)
            {
        ...
        var full = Visa2014ApplicationProfileInstanceProgressCompletionIndex.BuildVisaExtensionEvidence(
            new Dictionary<string, string?>(StringComparer.Ordinal)
            {
                ["ApplicationItemCount"] = "3",
                ["VisaLinkedCount"] = "3",
    ...
    public void Merge_PrefersInvitationWorkPermitOverVisaExtension()
    {
        ...
        var inv = new Visa2014ApplicationProfileInstanceProgressCompletionEvidence(
            new DateTime(2016, 1, 1), "InvitationNumber", "INV-1");
        var visa = new Visa2014ApplicationProfileInstanceProgressCompletionEvidence(
            new DateTime(2018, 5, 1), "VisaNumber", "V-1");

        var map = Visa2014ApplicationProfileInstanceProgressCompletionIndex.Merge(
            [(appOid, inv)],
            [(appOid, visa)]);
