...
internal sealed class Visa2014BorderZoneItemImportResult
{
...
internal static class Visa2014BorderZoneItemODataImporter
{
    public static async Task<Visa2014BorderZoneItemImportResult> RunAsync(
        ...
        string? borderZoneItemIdMapOutputPath,
        int? maxRows,
        bool dryRun,
        bool verbose)
    {
        ...
        var borderZoneIdMap = Visa2014IdMapHelper.Load(borderZoneIdMapPath);
        ...
        if (verbose)
        {
            ...
            Console.WriteLine($"INF BorderZone id-map entries: {borderZoneIdMap.Count}");
        }

        var batch = Visa2014BorderZoneItemTransform.PrepareImportBatch(
        ...
        if (dryRun)
        {
            int missing = CountMissingRequiredIdMap(batch.ImportRows, personIdMap, passportIdMap, borderZoneIdMap);
            Console.WriteLine(
                $"DRY RUN: {batch.ImportRows.Count} row(s) ready to POST " +
                $"({batch.Skipped.Count} skipped, {missing} missing required id-map).");
            return new Visa2014BorderZoneItemImportResult
            {
        ...
        var itemIdMap = LoadOptionalIdMap(borderZoneItemIdMapOutputPath);
        ...
        foreach (var row in batch.ImportRows)
        {
            ...
            if (itemIdMap.ContainsKey(legacyOid))
            {
            ...
            try
            {
                ...
                {
                    ["Person"] = new Dictionary<string, object?> { ["ID"] = personId },
                    ["Passport"] = new Dictionary<string, object?> { ["ID"] = passportId },
                    ["BorderZone"] = new Dictionary<string, object?> { ["ID"] = borderZoneId },
                };
                var createdId = await target.CreateAsync(typeof(Bo.BorderZoneItem), payload);
                ...
                itemIdMap[legacyOid] = createdId.Value;
                posted++;
                if (posted % 50 == 0)
        ...
        if (itemIdMap.Count > 0 && !string.IsNullOrWhiteSpace(borderZoneItemIdMapOutputPath))
        {
            idMapPath = Path.GetFullPath(borderZoneItemIdMapOutputPath);
            Directory.CreateDirectory(Path.GetDirectoryName(idMapPath)!);
            var serializable = itemIdMap.ToDictionary(
    ...
    private static int CountMissingRequiredIdMap(
        ...
        IReadOnlyDictionary<Guid, Guid> borderZoneIdMap)
    {
        int missing = 0;
        foreach (var row in importRows)
        {
            if (!TryResolveRequiredIds(row, personIdMap, passportIdMap, borderZoneIdMap, out _, out _, out _, out _))
                missing++;
        }
        ...
        out string missingReason)
    {
        ...
        if (!TryResolveLegacyGuid(row, "BorderZone", out var legacyBorderZoneOid) ||
            !borderZoneIdMap.TryGetValue(legacyBorderZoneOid, out borderZoneId))
        {
            missingReason = "BorderZone not in id-map";
            return false;
        }
