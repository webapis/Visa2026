---
title: Arza ösüşini yzarlamak
slug: applications/progress
locale: tk
tier: 4
guideStatus: draft
bo: ApplicationProgress
navPath: Applications
roles: [Visa Officer]
prerequisiteSlugs:
  - applications/overview
  - applications/create
  - applications/add-items
screenshotsVersion: "2026.08"
videoStorage: static
videoFile: application-progress.mp4
e2eScenarioId: person-officer-journey
verified: false
---

# Arza ösüşini yzarlamak

**Application progress** — arzanyň iş akymyndaky ädimleriň taryhy. Her geçiş üçin **täze setir** goşulýar.

!!! tip "Öňünden"
    [Applications gysgaça](overview.md), [Arza döretmek](create.md), [Application items goşmak](add-items.md).

## Esasy düşünje

| Boş taryh | Ofisde taýýarlanýar (örtük) |
| Soňky ýagdaý | **Progress history** iň soňky setiri |

| Ugur | Ilkinji setir |
|------|---------------|
| **via ministry** | Ministrlige iberildi |
| **direct migration** | Migrasiýada iş başlandy |

## Ädimler

1. Dogry başlyk sanawyndan arzany açyň.
2. **Progress** → **Progress history** → **New**.
3. **State**, **Date** → gerek bolsa **Ministry letter** → **Save**.

## Indiki

- [Applications gysgaça](overview.md)
- [Visa2026 mümkinçilikleri](../../about/capabilities.md)