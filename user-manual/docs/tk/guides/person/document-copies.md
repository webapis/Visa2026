---
title: Adam resminama nusgalary
slug: person/document-copies
locale: tk
tier: 6
guideStatus: draft
bo: Person
navPath: Person
roles: [Visa Officer, Visa Chief]
prerequisiteSlugs:
  - person/open-and-search
  - getting-started/navigation
screenshotsVersion: "2026.08"
videosVersion: "2026.08"
videoStorage: static
videoFile: person-document-copies.mp4
videoSource: recordings/person-master-data-journey.mp4
e2eScenarioId: person-officer-journey
verified: false
sourceDocs:
  - docs/PERSON_DOCUMENT_COPIES.md
---

# Adam resminama nusgalary

Şahsy ýazgynyň skanlaryny bir **önizleme panelinde** bölümler boýunça görüň. Ministrlik PDF ZIP üçin [Arza resminama nusgalary](../applications/document-copies.md).

## Giriş

| Ýer | Düwme |
|-----|-------|
| Detal formasy | **Person document copies** |
| Sanaw | **Dossier** den soň **•** sütun |
| Dosye | **Document copies** |

## Ädimler

1. Katalogy açyň.
2. **Ready** setirde **Preview**.
3. **Refresh** — maglumat üýtgedilenden soň.

[Şahsy dosýa](dossier.md).