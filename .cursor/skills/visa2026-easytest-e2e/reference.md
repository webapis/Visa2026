...
- **Snapshot (GitHub Actions):** matrix jobs `E2E — Sign in` / Register / Find / Add passport (`fail-fast: false`, **`max-parallel` default 4**). Override: workflow_dispatch `max_parallel` or repo var `E2E_MAX_PARALLEL`. Shared cache key; `VISA2026_E2E_SNAPSHOT_TRUST=true`. Local Facts stay serial (`DisableTestParallelization`, one `:5050`).
