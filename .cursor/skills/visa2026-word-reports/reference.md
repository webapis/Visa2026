...
**`App_Inv_And_WP_Letter` / `App_Visa_And_WP_Ext_Letter` ministry addressee:** `MinistryRecipientBlockFormatter.SplitIntoAddressLines` still supplies `Line1` / `Line2` / `HasLine2`. The Word template places them in a **full-width borderless table** (`AppendMinistryRecipientBlockRightColumnTable`): a **wide left spacer** cell and a **right-hand address** cell whose width is **capped** (`MinistryRecipientTableAddressColumnMaxTwips`, default **6400** twips, with **`MinistryRecipientTableAddressColumnMinTwips`** floor and **`MinistryRecipientTableMinSpacerTwips`** minimum on the spacer) so short recipient text sits on the **right** of the printable area; long lines wrap inside that column. Both lines are **`w:jc left`** inside the address cell so they share one **left edge** (avoid paired right-justified paragraphs, which stagger). DocxTemplater `{?{ds.ProjectContract_Ministry_RecipientBlock_HasLine2}}…{{/}}` on line 2 omits content when `HasLine2` is false.
