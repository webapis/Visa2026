...
**`App_Inv_And_WP_Letter` / `App_Visa_And_WP_Ext_Letter` ministry addressee:** `MinistryRecipientBlockFormatter.SplitIntoAddressLines` still supplies `Line1` / `Line2` / `HasLine2`. The Word template places them in a **full-width borderless table**: a **narrow spacer** cell (~900 twips) and an **address** cell that consumes the **remaining printable width** (~8600 twips at 1200/1200 margins) so long ministry lines do not wrap awkwardly. Both lines are **`w:jc left`** inside the address cell so they share one **left edge**, block on the **right**. DocxTemplater `{?{ds.ProjectContract_Ministry_RecipientBlock_HasLine2}}…{{/}}` on line 2 omits content when `HasLine2` is false.
