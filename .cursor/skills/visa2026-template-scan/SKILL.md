...
| Education Review shows one joined **EGIY** / **FMEIY** instead of level and institution | **EGLV** and **EGIN** are separate. Combined *Bilimi we okan ýeri* splits `Ýokary, …uniwersiteti`. Add list hides **EGIY** (type `EGIY` to reach it). Stop F5, rebuild, **Analyze** | **This skill** |
...
| Review Add Visa is one flat list | **Visa — linked active** (VNUM/VTYP/VSTD/VEDT…) vs **Visa — cancel** (CVNB/CVSB/CVEB). Number and type are separate — not **VNAT**. Requested period/category stay **Application — general**. Stop F5, rebuild, hard-refresh | **This skill** + user-report-templates |
| Review Add / Placeholder Manual cannot find salary currency or contract dates | Filter `Contract`, `CCUR`, `CSAL`, `CSDT`, `CEDT`, `currency`, or `şertnama`. **1.667.00 USD** is **CSAL** then **CCUR**. Date range with *Zähmet şertnamasy* is **CSDT** then **CEDT**. Stop F5, rebuild, hard-refresh | **This skill** + user-report-templates |
| Placeholder Manual groups differ from Review Add | Same officer list: Application / Visa subgroups, hide **EGIY**/**VNAT** unless typed, search aliases (`visa start` → **VSTD**). Filter shows only groups that have codes. Stop F5, rebuild, hard-refresh the catalog **Placeholder Manual** | **This skill** + user-report-templates |
| Excel Generate puts `{{#ds.rows}}` / `{{/ds.rows}}` between **ACPOS** and **ACFNM** | Those loop tags belong on the roster data row only. Footer signatory cells stay `{{ds.ACPOS}}` / `{{ds.ACFNM}}`. Stop F5, rebuild, **Generate** again | **This skill** |
| Review Add placeholder cannot find education (level, institution, specialty) | Filter `education`, `EGLV`, `EGIN`, `EGSP`, or `speciality`. **EGLV** and **EGIN** are separate — not joined **EGIY**. Cancel-visa still offers Education tokens; People & links Education tile stays hidden. Restart, hard-refresh | **This skill** |
