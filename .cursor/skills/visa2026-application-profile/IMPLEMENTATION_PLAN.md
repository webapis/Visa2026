...
| 8-loc | Layer A localize template + instance | **Done** | `ApplicationProfileLocalization` + `UiStrings.application-profile*.json`; catalog, wizard (all 5 steps), picker, overview, officer-shell crumbs, workspace header/nav (en/tr/tk/ru). Workspace org/progress/SLA body and Convert dialog still English. |
...
| 10x-gate | Case summary completeness gate | **Done** | Office preparation: empty (red) Case summary fields block Progress / documents / Resminamalar / SLA / Advance. People, links, Application result, and Organization stay open. Banner names missing fields. |
...
| 10q-result | Application result workspace tab | **Done** | Issued invitation / WP / visa / rejection / border zone moved off Overview to **Application result** (Ýüztutmanyň netijesi), after Progress. Same tiles + compose. |
| 10q-result-ov | Application result overview | **Done** | Per-type roster coverage + **%**. Do not sum people across types. Rejection optional. Hereketler still counts headers. |
| 10q-result-lv | Instance ListView result coverage | **Done** | Same people/expected chips as Netije syny in one **Ýüztutmanyň netijesi** column after Adam sany. Batch preload. Rejection optional. |
| 10q-progress-lv | Instance ListView progress stepper | **Done** | Compact workspace stepper replaces Ýagdaý text. Same slots as Progress tab. Batch history in preload. Netije stays last. |
| 10v | People & links New missing person-owned BO | **Deferred** | In-tab **New {type}** removed — officers add person-owned data from **Open person detail**. Issued items stay on Application result. `EnsureResolvedLink` kept for Relink. |
...
| 14 | **Seretmezlik** — exclude roster people after submission | **Done** (2026-09-24) | `ApplicationProfileInstanceExclusion` + `…ExclusionPerson` (schema heal `ApplicationProfileInstanceExclusionSchemaSql`); shared company counter (`ApplicationProfileInstanceExclusionNumbering`); `IApplicationProfileInstanceExclusionService` (gate after office prep, current holder from Progress, prefills, close as `PROCESS_CANCELLED`); Word letter `ApplicationProfileInstanceExclusionLetterBuilder` (built-in layout; company template per slice 14b); tab `OfficerShellCaseSeretmezlikTab.razor`; excluded people skipped by Resminamalar, Document copies, People-links completeness, and **Application Result** expected/coverage + ListView Netije chips (active roster only). Prototype: `docs/prototypes/application-profile-instance-seretmezlik-README.md`. |
| 14b | **Seretmezlik templates** — officer-editable Letter + Roster (Sanaw) | **Done** (2026-09-24) — Phase 1 | Company-wide `ApplicationProfileInstanceExclusionTemplate` (one row per `Kind` Letter/Roster, `TemplateFile` FileData; not `UserReportTemplate` so it stays out of the Resminamalar Shared pane). Templates pane in the Seretmezlik tab: Preview sample, Download current / built-in, Replace (upload `{{ds.*}}` .docx; roster .docx or .xlsx), Reset to built-in, placeholder manual. Upload validation: known tokens only, roster needs `{{#ds.People}}` row, trial merge. Documents regenerated on demand; own ClosedXML merger for the Excel roster. **Phase 2 (open):** Create from yellow marks / Add existing for Seretmezlik (profile-bound Scan/Convert dialogs need a non-profile placeholder set). |
...
| B5 | Case workspace tab shell | **Done** | Overview, people, progress, **Application result**, documents, Resminamalar, SLA, Organization last. Tab switch shows top + panel progress bar (same pattern as Report Dashboard) |
...
| 1 | **Done** | Tenant `application-profile.calik-energi.json` — `RequirePersonTravelHistory` **on** for **Registration** only |
