...
- **Past, not today:** all imported applications are historical. Roster links must match `PersonInApplication` snapshots (Passport / Visa / WorkPermitItem). Do **not** attach latest-N / `PersonCurrentItems` during import (`IsDataImport` skips auto-link). Pin documents at roster; `--correct-visa2014-application-person-document-links` also pins WorkPermitItem. **Calik (2026-09-14):** `App_Visa_and_WP_Ext` / `extend_visa_wp` requires the **WorkPermitItem being extended** (`RequirePersonWorkPermitItem`); re-run document-link correction after that flag turns on. Education / Position / AddressOfResidence / TravelHistory are pinned when the instance profile `RequirePerson*` is on — via PIA id-map when present, then a roster SQL backfill of each person's own imported child row (`--epa-roster-backfill-only` for that step alone). **Calik (2026-09-11):** Address and Position are required on **every** profile, including **App_Change_Inv**. After document-link correction, **always** run `--epa-roster-backfill-only`; **re-run EPA** if `RequirePerson*` turned on after a prior pin. Position pin is employees only. TravelHistory pin is Registration ActionFamily SQL only (`--travel-roster-backfill-only`). Officer Relink may still use today.
