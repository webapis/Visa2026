...
- **Past, not today:** all imported applications are historical. Roster links must match `PersonInApplication` snapshots (Passport / Visa / WorkPermitItem). Do **not** attach latest-N / `PersonCurrentItems` during import (`IsDataImport` skips auto-link). Pin documents at roster; `--correct-visa2014-application-person-document-links` also pins WorkPermitItem. Education / Position / AddressOfResidence / TravelHistory are pinned when the instance profile `RequirePerson*` is on — via PIA id-map when present, then a roster SQL backfill of each person's own imported child row (`--epa-roster-backfill-only` for that step alone). Officer Relink may still use today.
