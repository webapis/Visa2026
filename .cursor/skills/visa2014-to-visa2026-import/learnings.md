...
### 2026-09-23 — AddressOfResidence duplicates on PostgreSQL (site guard was skipped)

- **Symptom**: Same Person+Type+City+Lodging/Hotel/FullAddress rows repeated (up to 9x); 5292 AOR with 476 exact duplicate extras. Lodging/Hotel catalog also had a few FullAddress/Name twins from null-City sync misses.
- **Cause**: `Visa2014AddressOfResidenceSiteDuplicateGuard.LoadFromSqlAsync` and `TryMatchExistingAddressAsync` returned empty on PostgreSQL (SqlClient-only). Re-import / PIA inference posted again.
- **Fix**: Guard + TargetMatcher load/match via Npgsql; wire `siteGuard.TryResolveFromPayload` before create (main + PIA). Soft-deleted 476 AOR + 3 Lodging + 1 Hotel + 1 OtherSite extras (`GCRecord=1`) on local and `.26`. Lookup sync adds `RemoveDuplicateSiteNullCityOrphans`.
- **Prevent**: Do not re-import AddressOfResidence without target connection string (guard needs it). Prefer `--inprocess --target-connection` against Postgres.
