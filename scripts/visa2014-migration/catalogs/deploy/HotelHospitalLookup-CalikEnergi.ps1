...
<#
.SYNOPSIS
  Deploy Çalik Energi Hotel + Hospital tenant catalogs for local F5 / updateDatabase sync.
#>
param(
...
Copy-Item -Force (Join-Path $tenantDir 'hotel.json') (Join-Path $overlayDir 'hotel.json')
Copy-Item -Force (Join-Path $tenantDir 'hospital.json') (Join-Path $overlayDir 'hospital.json')
...
Write-Host ""
Write-Host "Next: sync catalogs (hotel + hospital rows) to LocalDB:"
