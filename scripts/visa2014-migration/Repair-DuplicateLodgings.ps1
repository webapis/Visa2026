...
<#
.SYNOPSIS
  Preview or soft-delete duplicate Lodging catalog rows (same FullAddress) on Visa2026 prod.

.DESCRIPTION
  Keeps the Lodging with the most AddressesOfResidence refs (then non-null City, then MIN(ID)).
  Repoints AddressesOfResidence / LodgingDocuments / LodgingImages, then soft-deletes extras (GCRecord = 1).
  SQL: cleanup/DuplicateLodgingsByFullAddress.sql
  Default is PREVIEW only. Pass -Apply after reviewing output.

.EXAMPLE
  .\scripts\visa2014-migration\Repair-DuplicateLodgings.ps1
.EXAMPLE
  .\scripts\visa2014-migration\Repair-DuplicateLodgings.ps1 -Apply
#>
...
$sqlPath = Join-Path $PSScriptRoot 'cleanup\DuplicateLodgingsByFullAddress.sql'
...
Write-Host "=== Duplicate Lodging by FullAddress ($mode) ===" -ForegroundColor Cyan
Write-Host "Target: $TargetServer / $TargetDatabase" -ForegroundColor DarkGray

if ($Apply) {
    if (-not $PSCmdlet.ShouldProcess($TargetDatabase, 'Soft-delete duplicate Lodgings')) { return }
...
if (-not $Apply) {
    ...
    Write-Host '  .\scripts\visa2014-migration\Repair-DuplicateLodgings.ps1 -Apply' -ForegroundColor Green
}