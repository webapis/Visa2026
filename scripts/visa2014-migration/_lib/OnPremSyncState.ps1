...
function Get-OnPremDuplicateRowDefinitions {
    @(
        ...
        @{ BO = 'ApplicationProgress'; G = "SELECT COUNT(*) FROM (SELECT ApplicationID, ProgressOrder FROM ApplicationProgresses WHERE (GCRecord IS NULL OR GCRecord = 0) AND ApplicationID IS NOT NULL GROUP BY ApplicationID, ProgressOrder HAVING COUNT(*) > 1) d"; E = "SELECT ISNULL(SUM(cnt - 1), 0) FROM (SELECT COUNT(*) cnt FROM ApplicationProgresses WHERE (GCRecord IS NULL OR GCRecord = 0) AND ApplicationID IS NOT NULL GROUP BY ApplicationID, ProgressOrder HAVING COUNT(*) > 1) x" }
...
function Get-OnPremFileRowDefinitions {
    @(
        ...
        @{ BO = 'PassportDocument'; L = 'SELECT COUNT(*) FROM dbo.PassportCopy pc INNER JOIN dbo.Passport p ON pc.Passport = p.Oid AND p.GCRecord IS NULL WHERE pc.GCRecord IS NULL AND pc.Passport IS NOT NULL'; T = 'SELECT COUNT(*) FROM PassportDocuments WHERE GCRecord IS NULL OR GCRecord = 0'; Map = 'PassportCopy' }
        @{ BO = 'EducationDocument'; L = 'SELECT COUNT(*) FROM dbo.PassportCopy pc INNER JOIN dbo.Education e ON pc.Education = e.Oid AND e.GCRecord IS NULL WHERE pc.GCRecord IS NULL AND pc.Education IS NOT NULL'; T = 'SELECT COUNT(*) FROM EducationDocument WHERE GCRecord IS NULL OR GCRecord = 0'; Map = 'EducationDocument' }
        @{ BO = 'VisaDocument'; L = 'DECLARE @c sysname = (SELECT TOP 1 name FROM sys.columns WHERE object_id = OBJECT_ID(N''dbo.Visa'') AND name LIKE N''G%rmeNusga''); DECLARE @sql nvarchar(max) = N''SELECT COUNT(*) FROM dbo.Visa v INNER JOIN dbo.Passport p ON v.Passport = p.Oid AND p.GCRecord IS NULL WHERE v.GCRecord IS NULL AND v.'' + QUOTENAME(@c) + N'' IS NOT NULL AND DATALENGTH(v.'' + QUOTENAME(@c) + N'') > 0''; EXEC sp_executesql @sql;'; T = 'SELECT COUNT(*) FROM VisaDocument WHERE GCRecord IS NULL OR GCRecord = 0'; Map = 'VisaDocument' }
        ...
        @{ BO = 'FamilyProofDocument'; L = 'SELECT COUNT(*) FROM dbo.FamilyProofDocument WHERE GCRecord IS NULL'; T = 'SELECT COUNT(*) FROM PersonFamilyRelationDocuments WHERE GCRecord IS NULL OR GCRecord = 0'; Map = 'FamilyProofDocument' }
