...
<#
.SYNOPSIS
  Preview or soft-delete duplicate ApplicationItems (same Person on same Application) on Visa2026 prod.

.DESCRIPTION
  Keeps the oldest row (MIN(ID)) per (Application, Person). Soft-deletes extras (GCRecord = 1).
  SQL: cleanup/DuplicateApplicationItemsByPerson.sql
...
.EXAMPLE
  ...
  .\scripts\visa2014-migration\Repair-DuplicateApplicationItems.ps1

.EXAMPLE
  .\scripts\visa2014-migration\Repair-DuplicateApplicationItems.ps1 -Apply
#>
...
$sqlPath = Join-Path $PSScriptRoot 'cleanup\DuplicateApplicationItemsByPerson.sql'
...
Write-Host "=== Duplicate ApplicationItems by Person ($mode) ===" -ForegroundColor Cyan
Write-Host "Target: $TargetServer / $TargetDatabase" -ForegroundColor DarkGray

if ($Apply) {
    if (-not $PSCmdlet.ShouldProcess($TargetDatabase, 'Soft-delete duplicate ApplicationItems')) {
        return
    }
...
if (-not $Apply) {
    ...
    Write-Host '  .\scripts\visa2014-migration\Repair-DuplicateApplicationItems.ps1 -Apply' -ForegroundColor Green
}
