...
<#
.SYNOPSIS
  Preview or soft-delete duplicate ApplicationProgress rows (same Application + ProgressOrder) on Visa2026 prod.

.DESCRIPTION
  Keeps the oldest row (MIN(ID)) per (Application, ProgressOrder). Soft-deletes extras (GCRecord = 1).
  SQL: cleanup/DuplicateApplicationProgressByAppOrder.sql
  Default is PREVIEW only. Pass -Apply after reviewing output.

.EXAMPLE
  .\scripts\visa2014-migration\Repair-DuplicateApplicationProgress.ps1
.EXAMPLE
  .\scripts\visa2014-migration\Repair-DuplicateApplicationProgress.ps1 -Apply
#>
...
$sqlPath = Join-Path $PSScriptRoot 'cleanup\DuplicateApplicationProgressByAppOrder.sql'
...
Write-Host "=== Duplicate ApplicationProgress by App+Order ($mode) ===" -ForegroundColor Cyan
Write-Host "Target: $TargetServer / $TargetDatabase" -ForegroundColor DarkGray

if ($Apply) {
    if (-not $PSCmdlet.ShouldProcess($TargetDatabase, 'Soft-delete duplicate ApplicationProgress')) { return }
...
if (-not $Apply) {
    ...
    Write-Host '  .\scripts\visa2014-migration\Repair-DuplicateApplicationProgress.ps1 -Apply' -ForegroundColor Green
}