...
<#
.SYNOPSIS
  Import VISA2014 WorkPermit headers then WorkPermitItem rows (in-process).

.EXAMPLE
  .\scripts\visa2014-migration\import\WorkPermits.ps1 -MaxRows 50
  .\scripts\visa2014-migration\import\WorkPermits.ps1
  .\scripts\visa2014-migration\import\WorkPermits.ps1 -DryRun
#>
[CmdletBinding()]
param(
...
if (-not $ItemsOnly) {
    Invoke-Visa2014Import -Entity "WorkPermit" -ExtraArgs @(
        "--id-map-output", (Join-Path $mapRoot "WorkPermit.json")
    )
}
