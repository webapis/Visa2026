#!/bin/bash
# SQL-first prod deploy for Linux on-prem (Ubuntu VM).
# Usage: sudo bash remote-compose-sql-up.sh [deploy-root]
set -euo pipefail

DEPLOY_ROOT="${1:-/opt/visa2026}"
cd "$DEPLOY_ROOT"

if [ ! -f .env.prod ]; then
  echo "ERROR: missing $DEPLOY_ROOT/.env.prod" >&2
  exit 1
fi
