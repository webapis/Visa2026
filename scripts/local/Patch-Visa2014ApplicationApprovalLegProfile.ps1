...
.EXAMPLE
  .\scripts\local\Patch-Visa2014ApplicationApprovalLegProfile.ps1 -DryRun
  .\scripts\local\Patch-Visa2014ApplicationApprovalLegProfile.ps1 -ApiBaseUrl "https://localhost:5001"
#>
[CmdletBinding()]
param(
    ...
    [string]$ImportPassword = $(if ($env:VISA2026_IMPORT_PASSWORD) { $env:VISA2026_IMPORT_PASSWORD } else { "" }),
    [int]$MaxRows = 0,
