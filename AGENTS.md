...
- **`.cursor/skills/visa2026-onprem-legacy-sync/SKILL.md`** (+ **`reference.md`**, **`user-prompts.md`**, **`learnings.md`**, **`MATURITY.md`**) — optional Agent **Skill**: **one-way legacy sync** `VISA2015` on `10.100.128.15` → **Visa2026 prod only** on `10.100.128.25` (staging = prod backup restore; manual catch-up then nightly on `.25`, cutover ~1–2 months); MCP **`visa2014-sql-remote`**; `OnPrem-Sync.ps1 -Profile Production`; runbook **`docs/VISA2014_MIGRATION/ON_PREM_IIS_MIGRATION_RUNBOOK.md`**. Not BO discovery (**visa2014-to-visa2026-import**) or IIS publish (**visa2026-windows-iis-deploy**).
- **On-prem skill maturity** — **setup-docker-engine**, **setup-openssh-server**, **legacy-on-prem-windows-setup**, **visa2026-windows-iis-deploy**, and **visa2026-onprem-legacy-sync** **accumulate experience** in each folder’s **`learnings.md`** (read before work, append after verified fixes). Shared loop and promotion rules: **`.cursor/skills/on-prem-deploy/MATURITY.md`**.
...
- **`.cursor/skills/visa2014-to-visa2026-import/SKILL.md`** (+ **`MATURITY.md`**, **`reference.md`**, **`import-practices.md`**, append-only **`learnings.md`**) — optional Agent **Skill**: [IMPORT_PLAN](docs/VISA2014_MIGRATION/IMPORT_PLAN_AND_STRATEGY.md) before import code; **Excel preview** of consolidated legacy data; read/append **learnings** each session (**every import attempt**, success or failure); **reuse** `scripts/visa2014-migration/` before new scripts; per-BO **`importConfirmed`** — not **visa2026-dataimporter** seed scenarios.
