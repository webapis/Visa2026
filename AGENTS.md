...
- **`.cursor/skills/carbone/SKILL.md`** (+ **`references/`**, append-only **`learnings.md`**) — third-party **Carbone** templating syntax (`{d.…}`, formatters, loops); use on **`carbone` branch** / migration spikes — **not** DocxTemplater `{{ds.*}}` unless migrating merge engine. Upstream: [carboneio/carbone-skill](https://github.com/carboneio/carbone-skill). Canonical plan: **[`docs/CARBONE_INTEGRATION_PLAN.md`](docs/CARBONE_INTEGRATION_PLAN.md)**.
