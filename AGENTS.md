...
- **`.cursor/skills/visa2026-ui-test-hooks/SKILL.md`** (+ **`reference.md`**, **`registry.md`**, append-only **`learnings.md`**) — optional Agent **Skill**: **Blazor UI test hooks** (`data-testid`, `InputId`, `e2e-*` classes via Module `ModelDefault`, `Model.xafml`, Blazor controllers); DevTools verify; stage smoke / Playwright; not EasyTest CI suite.
