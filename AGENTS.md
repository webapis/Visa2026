...
- **`.cursor/skills/visa2026-ui-scenarios/SKILL.md`** (+ **`reference-map-contract.md`**, **`reference.md`**, **`examples/*_map.md`**, **`examples/*.yaml`**, **`learnings.md`**) — **Map → Hooks → YAML**: `<id>_map.md` plans journey + hook gaps; **ui-test-hooks** fills gaps; then `<id>.yaml` for Playwright (**`tools/UiScenarioRunner`** planned).
