...
- **`docs/PERSON_INCOMPLETE_DATA.md`** — officer soft **Incomplete** flag on `Person` (checkboxes + notes, Mark incomplete / Mark complete); Report Dashboard **Incomplete persons** category (orthogonal to automatic State notifications completeness).
...
- **`.cursor/rules/*.mdc`** — Cursor-only rules (always-on core + file-scoped Module / Blazor host). Same intent as this file, kept short for the agent. **No Domain Components:** `.cursor/rules/visa2026-no-domain-components.mdc`.
...
- **`.cursor/skills/visa2026-resminamalar/SKILL.md`** (+ **`prompts.md`**, **`reference.md`**, **`MATURITY.md`**, append-only **`learnings.md`**) — optional Agent **Skill**: **Resminamalar** report package dialog (catalog, preview, ZIP, **desktop template staging**); **read learnings before / append after verified fixes**; scenarios promote from experience. Canonical **`docs/APPLICATION_REPORT_PACKAGE.md`**; staging detail **`docs/TEMPLATE_STAGING_EDIT.md`**.
- **`.cursor/skills/visa2026-preview-slot/SKILL.md`** (+ **`reference.md`**, **`prompts.md`**, **`MATURITY.md`**, append-only **`learnings.md`**) — optional Agent **Skill**: global **`#visa-preview-slot`** shell (Resminamalar, Document copies, Progress letters, file preview); catalog card UX, exclusive preview mode, resize/theme/occupants — not ZIP/readiness domain logic. Canonical **`docs/PREVIEW_SLOT.md`**.
- **`.cursor/skills/visa2026-user-report-templates/SKILL.md`** (+ **`reference*.md`**, append-only **`learnings.md`**) — optional Agent **Skill**: user report template **seeds** under **`Resources/Templates/`** (Word/Excel embed, maps, Extract/Validate, merge families); not dialog UX (see **resminamalar**).
- **`.cursor/skills/visa2026-word-reports/SKILL.md`** (+ **`reference.md`**, **`review-status.md`**) — **deprecated** for new work (code-backed `IWordReportDefinition` removed); historical **`docs/WORD_REPORT_GENERATION_IDEA.md`** / XtraReports notes only.
- **`docs/REPORT_DASHBOARD.md`** — officer Report Dashboard home (charts, Excel, ListView); supersedes removed State Dashboard UI.
- **`.cursor/skills/visa2026-report-dashboard/SKILL.md`** (+ **`reference.md`**, **`IMPLEMENTATION_PLAN.md`**, append-only **`learnings.md`**) — optional Agent **Skill**: **Report Dashboard** prototyping and evolution: add categories/sub-reports, wire mock → SQL view → real `ReportDashboardQueryService`, **Preview ↔ `vw_rd_*` ↔ XAF ListView parity** (one view + ListView per subreport; Excel same population), status CSS classes. Canonical **`docs/REPORT_DASHBOARD.md`**.
- **`.cursor/skills/visa2026-analytics-ai-chat/SKILL.md`** (+ **`reference.md`**, **`IMPLEMENTATION_PLAN.md`**, **`prompts.md`**, append-only **`learnings.md`**) — optional Agent **Skill**: officer **AI analytics chat** on Report Dashboard (allow-listed aggregate tools, `None` / Azure providers, panel UX, audit turns). Not `vw_rd_*` promotion (**visa2026-report-dashboard**) or Template Convert AI. Canonical **`docs/ANALYTICS_AI_CHAT.md`** (planned with slices).
- **`docs/STATE_NOTIFICATIONS_IMPLEMENTATION_PLAN.md`** — State notifications inbox (header bell, validity + missing-data alerts, phases after UI prototype); links to state evaluation docs.
...
- **[`docs/APPLICATION_PROGRESS_APPROVAL_AND_CONTRACT_DEPTH.md`](docs/APPLICATION_PROGRESS_APPROVAL_AND_CONTRACT_DEPTH.md)** — approval step flows (ministry legs, direct migration), `ProjectContract` ministry legs, resolver and business rules.
- **`.cursor/skills/visa2026-application-progress/SKILL.md`** (+ **`reference.md`**, **`prompts.md`**, **`MATURITY.md`**, append-only **`learnings.md`**) — optional Agent **Skill**: **`ApplicationProgress`** workflow (transitions, contract/ministry legs, ministry letter file, validation, unit tests) — not ListView row colors (**visa2026-bo-state-colors**).
- **[`docs/APPLICATION_PROFILE_PLAN.md`](docs/APPLICATION_PROFILE_PLAN.md)** — **Application Profile** live FK model, prototypes, Person M2M (retire ApplicationItem), Person/Dossier start-application; **`ApplicationType` deprecated**.
- **`.cursor/skills/visa2026-application-profile/SKILL.md`** (+ **`reference.md`**, **`IMPLEMENTATION_PLAN.md`**, **`prompts.md`**, **`MATURITY.md`**, append-only **`learnings.md`**) — **sole** Agent **Skill** for Application Profile implementation: seed/cutover, wizard, picker, config lock, officer configuration suggestions, slice tracking — not progress transitions (**visa2026-application-progress**), Resminamalar (**visa2026-resminamalar**), or **Create from yellow marks** (**visa2026-template-scan**). Responsive case-workspace layout: **visa2026-responsive-ui**.
- **`.cursor/skills/visa2026-responsive-ui/SKILL.md`** (+ **`reference.md`**, **`prompts.md`**, **`MATURITY.md`**, append-only **`learnings.md`**) — officer shell responsive layout: minimum **1280×800**, Overview reflow, tile stretch, XAF left menu on instance detail. Not profile domain rules (**visa2026-application-profile**) or preview-slot shell (**visa2026-preview-slot**).
- **`.cursor/skills/visa2026-template-scan/SKILL.md`** (+ **`reference.md`**, **`prompts.md`**, **`MATURITY.md`**, append-only **`learnings.md`**) — **Create from yellow marks**: improve **Word and Excel** merge templates from yellow-marked Word/Excel (OpenXML → tokens on a copy → Approve). PNG/JPG/PDF retired. Not Convert (`TEMPLATE_AI_CONVERT_*`), Resminamalar ZIP, or embedding `#visa-preview-slot` inside the scan wizard. Canonical **`docs/TEMPLATE_AI_SCAN_PRODUCT_SPEC.md`**.
- **[`docs/TEMPLATE_AI_CONVERT_PRODUCT_SPEC.md`](docs/TEMPLATE_AI_CONVERT_PRODUCT_SPEC.md)** — **planned**: officer uploads a filled Word/Excel and it is auto-converted into a profile merge template (candidate check + highlights, no mapping grid, mapping-only chat). Locked officer decisions **L1–L13** (L13 = per-user convert editor switch on the instance); 16 UX prototypes under `docs/prototypes/template-ai-convert-*.png`.
- **[`docs/TEMPLATE_AI_CONVERT_UI_FLOW.md`](docs/TEMPLATE_AI_CONVERT_UI_FLOW.md)** — **interaction scenario**: views **V0–V11**, every control and where it leads, guards (suitability, validation, config lock), discard prompt, confirm dialog, transition map. Implemented in the officer-shell prototype (slice **E7a**); build the Blazor lift (**E7b**) against this, not the PNGs.
- **[`docs/TEMPLATE_AI_CONVERT_ENGINEERING_SPEC.md`](docs/TEMPLATE_AI_CONVERT_ENGINEERING_SPEC.md)** — **read before implementing** the above: Phase 0 service contracts (profile-scoped placeholder set, instance value map, token writer + diff gate, conversion draft, pluggable AI provider, ephemeral validate), locked **E-D1–E-D8**, slices **E0–E10** in the Application Profile tracker. Matching runs **locally**; the AI provider never receives raw instance values.
- **[`docs/DOCUMENT_SCAN_FIELD_EXTRACTION.md`](docs/DOCUMENT_SCAN_FIELD_EXTRACTION.md)** — **planned**: propose BO field values **from** an uploaded document copy (`Passport` first via **MRZ**, then `Visa`, `Invitation`/`WorkPermit` verification-only, `Education`), officer review, then save BO + `FileData` together. Locked **X-D1–X-D13**, slices **X0–X9**; 9 UI prototypes in [`docs/prototypes/document-scan-README.md`](docs/prototypes/document-scan-README.md). **Local-first** — scan bytes do not leave the host by default. Form **layouts are assumed to change** (X-D12: versioned data-driven layout profiles, anchor-relative locators, no pixel geometry). Opposite direction from `PdfFormMapping`; not template authoring (**TemplateScan** / **TemplateConvert**).
...
- **`.cursor/skills/visa2026-document-copies/SKILL.md`** (+ **`prompts.md`**, **`reference.md`**, **`MATURITY.md`**, append-only **`learnings.md`**) — optional Agent **Skill**: **Document copies** dialog (scan preview, package enqueue, PDF toast); **not** XFA mapping — see **pdf-form-mapping**. Canonical **`docs/APPLICATION_ITEM_DOCUMENT_COPIES.md`**. **ApplicationItem scope** (ministry ZIP). Person master-data preview: **`visa2026-person-document-copies`** + **`docs/PERSON_DOCUMENT_COPIES.md`** (Phases 1–2 shipped; 3–4 deferred).
- **`.cursor/skills/visa2026-person-document-copies/SKILL.md`** (+ **`reference.md`**, **`prompts.md`**, **`MATURITY.md`**, append-only **`learnings.md`**) — optional Agent **Skill**: **Person-scoped** document copies in preview slot (sectioned catalog, DetailView + ListView entry, preview-first). Phases 3–4 (ApplicationItem cross-link, person ZIP) **deferred**. Ministry ZIP: **document-copies** on ApplicationItem. Canonical **`docs/PERSON_DOCUMENT_COPIES.md`**.
- **`.cursor/skills/visa2026-person-dossier/SKILL.md`** (+ **`prompts.md`**, **`reference.md`**, **`MATURITY.md`**, append-only **`learnings.md`**) — optional Agent **Skill**: **Person dossier** (dosye) read-only 360 page, Screen/Paper, staged loading, director `PersonExportBatch` ZIP; Phase 2 deep-link open. Not Person search (**visa2026-report-dashboard**) or copies catalog (**visa2026-person-document-copies**). Canonical **`docs/PERSON_DOSSIER.md`**.
- **`.cursor/skills/visa2026-invitation-work-permit-document-copies/SKILL.md`** (+ append-only **`learnings.md`**) — optional Agent **Skill**: **Header & item** document copies for **Invitation, WorkPermit, Rejection, BorderZone**. **Border zone:** `BorderZoneDocument` + parent `Documents` (like Invitation/WorkPermit) ships **same release** as preview. **Planned**. **`docs/INVITATION_WORK_PERMIT_DOCUMENT_COPIES.md`**.
...
- **`docs/USER_MANUAL_ROADMAP.md`** — manual timeline, phases, milestones.
- **`docs/USER_MANUAL_E2E_MEDIA.md`** — contract linking guides to E2E scenarios, `Record-EasyTest.ps1`, and asset paths.
- **`.cursor/skills/visa2026-user-manual/SKILL.md`** (+ **`localization.md`**, **`content-policy.md`**, **`advisory.md`**, **`tracking.md`**, …) — officer manual **en/tr/tk/ru**; advise before implement; pairs with **visa2026-easytest-e2e** for media.
- **[`docs/VISA2014_MIGRATION.md`](docs/VISA2014_MIGRATION.md)** — **VISA2014 → Visa2026** prod data migration plan (MCP air gap, mapping YAML, OData import).
- **[`docs/VISA2014_MIGRATION/IMPORT_PLAN_AND_STRATEGY.md`](docs/VISA2014_MIGRATION/IMPORT_PLAN_AND_STRATEGY.md)** — import waves, environments, cutover; **approve before `--import-visa2014` implementation**.
- **[`docs/VISA2014_MIGRATION/EXCEL_PREVIEW_EXPORT.md`](docs/VISA2014_MIGRATION/EXCEL_PREVIEW_EXPORT.md)** — consolidated **`VISA2015` → Excel** to preview import content before OData (scalar only; no binary bytes).
- **[`docs/VISA2014_MIGRATION/FILE_AND_IMAGE_IMPORT.md`](docs/VISA2014_MIGRATION/FILE_AND_IMAGE_IMPORT.md)** — **binary file/image import** as a separate wave after owning BO id-map (Photo, scans, attachments).
- **[`docs/VISA2014_MIGRATION/LOOKUP_RESOLUTION_STRATEGY.md`](docs/VISA2014_MIGRATION/LOOKUP_RESOLUTION_STRATEGY.md)** — **do not import lookups**; translate legacy values to Visa2026 catalogs (gaps, duplicates, normalization).
- **[`docs/VISA2014_MIGRATION/lookup-comparisons/`](docs/VISA2014_MIGRATION/lookup-comparisons/)** — legacy vs Visa2026 lookup side-by-side (start with MaritalStatus).
- **[`docs/VISA2014_MIGRATION/STATUS.md`](docs/VISA2014_MIGRATION/STATUS.md)** · **[`migration-status.yaml`](docs/VISA2014_MIGRATION/migration-status.yaml)** — migration **done / in progress / issues** tracker.
- **`.cursor/skills/visa2014-to-visa2026-import/SKILL.md`** (+ **`MATURITY.md`**, **`reference.md`**, **`import-practices.md`**, **`user-prompts.md`**, append-only **`learnings.md`**) — **sole** Agent **Skill** for data migration (see bullet above under Further docs / Agent skills).
