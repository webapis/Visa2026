...
- **Multiple Individuals:** Each `Application` can contain one or more individuals, managed through the `PersonsInApplication` collection.
...
- **`PersonsInApplication` (PersonInApplication)**: A one-to-many, aggregated collection of individuals included in this application.
...
The following table details which properties are visible for each Application Type. Note that core properties like `ApplicationNumber`, `ApplicationDate`, `Status`, `Urgency`, `PersonsInApplication`, and `ApplicationResults` are generally visible for all types unless otherwise noted.
