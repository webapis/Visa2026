...
The `WorkPermit` business object is designed to manage the lifecycle of an employee's work permit. It stores all essential details, including the permit's validity dates, the associated employee and position, and references to the official documents and application processes that authorized it.
...
| `WorkPermitLetter` | `WorkPermitLetter` (Lookup) | A reference to the official letter that granted the permit. | Optional. |
...
- **`WorkPermitLetter` (WorkPermitLetter)**: A lookup relationship to the official approval letter.
