...
The visibility of properties within the `ApplicationItem` object is dynamically controlled by the `SubType` of the parent `Application`. This ensures that for any given application process, users are only presented with the fields relevant to the specific individuals involved. The matrix below details which properties are visible for each major application type.
