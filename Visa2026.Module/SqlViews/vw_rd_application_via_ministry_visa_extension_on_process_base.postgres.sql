-- Visa Extension on Process base.
-- One row per roster line (ApplicationProfileInstancePerson M2M + legacy ApplicationItem fallback) — PostgreSQL.
DROP VIEW IF EXISTS vw_rd_application_via_ministry_visa_extension_on_process_base CASCADE;
CREATE VIEW vw_rd_application_via_ministry_visa_extension_on_process_base AS
...
INNER JOIN "ApplicationProfiles" apf
   ...
   AND COALESCE(apf."ProduceVisa", FALSE) = TRUE
