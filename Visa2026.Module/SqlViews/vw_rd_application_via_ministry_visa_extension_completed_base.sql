...
CREATE OR ALTER VIEW [dbo].[vw_rd_application_via_ministry_visa_extension_completed_base] AS
SELECT
    ai.ID                                                               AS ID,
    ...
    COALESCE(NULLIF(LTRIM(RTRIM(v_on_ext.VisaNumber)), N''), N'')           AS VisaOnExtensionNumber,
    issued.ID                                                               AS IssuedVisaOid,
    COALESCE(NULLIF(LTRIM(RTRIM(issued.VisaNumber)), N''), N'')             AS IssuedVisaNumber,
    COALESCE(
        NULLIF(LTRIM(RTRIM(a.FullApplicationNumber)), N''),
        NULLIF(LTRIM(RTRIM(a.ApplicationNumber)), N''),
        N''
...
INNER JOIN ApplicationTypes at
   ...
   AND at.Name IN (N'App_Visa_Ext', N'App_Visa_Ext_According_to_WP', N'App_Visa_Ext_FM', N'App_Visa_and_WP_Ext')
...
LEFT JOIN Visas v_on_ext
    ON v_on_ext.ID = ai.CurrentVisaId
   AND ISNULL(v_on_ext.GCRecord, 0) = 0
OUTER APPLY (
    SELECT TOP 1 v.ID, v.VisaNumber
    FROM Visas v
    WHERE v.IssuingApplicationItemID = ai.ID
      AND ISNULL(v.GCRecord, 0) = 0
    ORDER BY v.IssueDate DESC, v.ID DESC
) issued
