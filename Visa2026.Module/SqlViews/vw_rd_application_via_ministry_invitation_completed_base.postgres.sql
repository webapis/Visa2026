...
SELECT
    ...
    COALESCE(NULLIF(BTRIM(issued_inv."InvitationNumber"), ''), '')           AS "InvitationNumber",
...
-- One invitation per ApplicationItem is a business rule, but LIMIT 1 keeps a data
-- anomaly from fanning out rows and breaking Preview/ListView total parity.
LEFT JOIN LATERAL (
    SELECT inv."ID", inv."InvitationNumber"
