-- Invitation on Process (P).
-- One row per roster line (ApplicationProfileInstancePerson M2M + legacy ApplicationItem fallback) — PostgreSQL.
DROP VIEW IF EXISTS vw_rd_application_via_ministry_invitation_on_process CASCADE;
CREATE VIEW vw_rd_application_via_ministry_invitation_on_process AS
...
INNER JOIN "ApplicationProfiles" apf
   ...
   AND COALESCE(apf."ProgressRoute", 0) = 0
...
WHERE COALESCE(a."GCRecord", 0) = 0
  AND NOT EXISTS (SELECT 1 FROM "Invitations" inv WHERE inv."ApplicationProfileInstanceID" = a."ID" AND COALESCE(inv."GCRecord", 0) = 0)
  AND (
