# Report Map — App_Business_Trip_Departure_app.jpg
...
| Field | Value |
|---|---|
| Class Name | `AppBusinessTripDepartureReport` |
| Registered Name | `"App Business Trip Departure Report"` |
| Display Name (Tm) | `"Iş Sapary — Gidiş"` |
| Reference Image | `App_Business_Trip_Departure_app.jpg` |
...
| Visibility Criteria | `[ApplicationType.Name] = 'App_Business_Trip_Departure'` |
...
| Detail | ~700F | Mostly inherited; derived adds Body3 (long), repositions Body2 |
| ReportFooter | 50F | Inherited — signatory labels |
| BottomMargin | 100F | Inherited |

> Detail is taller than Arrival because `ProjectContract_Description` (Maksady) spans many lines.
...
| `xrRichBody3` | XRRichText | (0F, 401F+gap) | (626.7717F, 80F) | **New in derived** | Maksady paragraph (see below) | CanGrow; Y = Body1.Bottom + 8F — grows long |
...
daşary ýurt raýatlarynyň
**[BusinessTripStartDateText]-den [BusinessTripEndDateText]-ne**
çenli **[BusinessTripDurationDays] gün** möhlet bilen
**[ToRegionName_Genitive]**
iş saparyna **gidýändigini** size habar berýäris.
```

> Note: Departure uses "raýatlarynyň" (plural genitive) and "gidýändigini" (going). Arrival uses "raýatlynyň" (singular) and "gelendigini" (arrived). Destination for Departure is region only (no city/district).

---

### Body3 RTF (Maksady Paragraph — long)

RTF paragraph — justified, first-line indent 0.5″, Times New Roman 15pt.

**Text structure:**
```
**Maksady:** [ProjectContract_Description]
```

`"Maksady:"` is bold inline; the full project contract description follows on the same paragraph. This paragraph grows to fit the full description text.
...
| Handwritten signature + date | Physical — do not replicate |
...
| `ToRegionName_Genitive` | ✅ | "Mary welaýatynyň" |
| `ProjectContract_Description` | ✅ | Maksady paragraph |
