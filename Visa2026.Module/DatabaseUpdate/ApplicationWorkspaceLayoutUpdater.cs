...
public sealed class ApplicationWorkspaceLayoutUpdater : ModelNodesGeneratorUpdater<ModelDetailViewLayoutNodesGenerator>
{
    public override void UpdateNode(ModelNode node)
    {
        if (FindDetailView(node) is not { } detailView
            || detailView.Id != ApplicationWorkspaceViewIds.DetailView)
        {
            return;
        }

        var layoutItem = FindLayoutViewItem(node, nameof(ApplicationWorkspaceHost.WorkspaceUi));
        if (layoutItem != null)
