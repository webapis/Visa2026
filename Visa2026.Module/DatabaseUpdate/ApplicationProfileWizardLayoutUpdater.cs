...
public sealed class ApplicationProfileWizardLayoutUpdater
    : ModelNodesGeneratorUpdater<ModelDetailViewLayoutNodesGenerator>
{
    public override void UpdateNode(ModelNode node)
    {
        if (FindDetailView(node) is not { } detailView
            || detailView.Id != ApplicationProfileWizardViewIds.DetailView)
        {
            return;
        }

        var layoutItem = FindLayoutViewItem(node, nameof(ApplicationProfileWizardHost.WizardUi));
        if (layoutItem != null)
