...
public sealed class ApplicationProfileOverviewLayoutUpdater
    : ModelNodesGeneratorUpdater<ModelDetailViewLayoutNodesGenerator>
{
    public override void UpdateNode(ModelNode node)
    {
        if (FindDetailView(node) is not { } detailView
            || detailView.Id != ApplicationProfileOverviewViewIds.DetailView)
        {
            return;
        }

        var layoutItem = FindLayoutViewItem(node, nameof(ApplicationProfileOverviewHost.OverviewUi));
        if (layoutItem != null)
