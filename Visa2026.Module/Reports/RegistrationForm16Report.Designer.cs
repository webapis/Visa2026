...
namespace Visa2026.Module.Reports
{
    partial class RegistrationForm16Report
    {
        ...
        private void InitializeComponent()
        {
            ...
            const float bw         = 2.5F;   // Main borders
            const float bwLow      = 2.0F;   // Lower tables borders
            ...
            xrPicturePhoto.ExpressionBindings.Add(new ExpressionBinding("BeforePrint", "Image", "[Person_Photo]"));
            ...
            BuildMainTable(xrTableMain, mainTableW, bw, 24F);
            ...
            xrLabelSignature.BorderWidth   = 2.5F;
            ...
            // Dashed Separator
            xrLineSeparator.LocationFloat = new PointFloat(0, sigY + 30F);
            xrLineSeparator.SizeF         = new SizeF(pageW, 3);
            xrLineSeparator.LineStyle     = DXDashStyle.Dash;
            xrLineSeparator.LineWidth     = 2F;
            xrLineSeparator.ForeColor     = bc;

            // Lower Tables
            float currentY = sigY + 70F;
            BuildRegistrationTable(currentY, bwLow, bc); currentY += 148F;
            BuildTravelTable(currentY, bwLow, bc);       currentY += 128F;
            BuildBottomTable(currentY, bwLow, bc);
        ...
        private void BuildMainTable(XRTable table, float tW, float bw, float rH)
        {
            ...
            MSpan(table, tW, 42F, bw, "[Person_ForeignAddress]", true);
            ...
            MSpan(table, tW, 26F, bw, "[Person_CompanyAddress]", true);
        }

        // Helper Methods with correct BorderDashStyle
        private void MTwo(XRTable t, float tW, float h, float bw, string label, string expr)
        {
            ...
            row.Cells.Add(MkExprCell(expr, tW - lW, bw));
            t.Rows.Add(row);
        }
        ...
        private void MFour(XRTable t, float tW, float h, float bw, string l1, string e1, string l2, string e2)
        {
            ...
            row.Cells.Add(MkExprCell(e1, w2, bw));
            row.Cells.Add(MkLblCell(l2, w3, bw));
            row.Cells.Add(MkExprCell(e2, w4, bw));
            t.Rows.Add(row);
        }
        ...
        private void BuildRegistrationTable(float y, float bw, Color bc)
        {
            ...
            string[] regHdr = { "Hasaba alan, möhletini uzaldan TDMG-nyň edarasy", "Hasaba alyş, uzaldyş belgisi", "Hasaba alnan, uzaldylan wagty", "Möhleti", "Esas(belgisi we wagty)", "Jogapkär işgäriň familiýasy we goly" };

            AddTableRow(xrTableRegistration, rW, 50F, bw, regHdr, null, true);
            AddTableRow(xrTableRegistration, rW, 30F, bw, null, new[] { "[Registration_TDMG]", "", "[Registration_DateText]", "[Registration_ExpiryText]", "[Registration_BasisText]", "" }, false);
            AddTableRow(xrTableRegistration, rW, 30F, bw, null, null, false);
            AddTableRow(xrTableRegistration, rW, 30F, bw, null, null, false);
        ...
        private void BuildTravelTable(float y, float bw, Color bc)
        {
            ...
            AddTableRow(xrTableTravel, tW, 38F, bw, trvHdr, null, true);
            AddTableRow(xrTableTravel, tW, 50F, bw, null, new[] { "[Address_FullAddress]", "[Travel_CheckPointTm]", "[Person_CompanyName]" }, false);
            AddTableRow(xrTableTravel, tW, 30F, bw, null, null, false);
        ...
        private void BuildBottomTable(float y, float bw, Color bc)
        {
            ...
            AddSpanRow(xrTableBottom, 767, 25, bw, "Hasapdan aýyrmak üçin esas", true);
            AddSpanRow(xrTableBottom, 767, 25, bw, "Başga bellikler", true);
        ...
        {
            var style = DXFontStyle.Bold | (underline ? DXFontStyle.Underline : 0);
            var row = new XRTableRow { HeightF = h };
            row.Cells.Add(new XRTableCell
            {
                Text = text,
