...
{
    /// <summary>
    /// Application-level registration request letter for App_Reg_Check_In.
    /// "Hatymyzyň goşundysynda görkezilen sanawdaky ... hasaba almagy'ňyzy Sizden haýyş edýäris."
    /// Template: Visa2026.Module.Resources.App_Reg_Check_In_Letter.docx
    /// </summary>
    public class AppRegCheckInLetterReportDef : IWordReportDefinition
    {
        public string[] ApplicableApplicationTypeNames => new[] { "App_Reg_Check_In" };

        public bool IsApplicable(Application application) => true;

        public string GetFileName(Application application) =>
            $"HasabaAlmak_{application.FullApplicationNumber}_{DateTime.Now:yyyyMMdd}.docx";
        ...
        public Task GenerateAsync(Application application, IWordFormFillerService wordService, Stream outputStream)
        {
            ...
            var asm = typeof(AppRegCheckInLetterReportDef).Assembly;
            const string res = "Visa2026.Module.Resources.App_Reg_Check_In_Letter.docx";
