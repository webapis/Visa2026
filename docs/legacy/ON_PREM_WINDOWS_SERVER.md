...
**Agent skills (split):** [**legacy-on-prem-windows-setup**](../.cursor/skills/legacy-on-prem-windows-setup/SKILL.md) (prereq audit + WSL/Ubuntu/systemd) · [**setup-openssh-server**](../.cursor/skills/setup-openssh-server/SKILL.md) (optional SSH) · [**setup-docker-engine**](../.cursor/skills/setup-docker-engine/SKILL.md) (Docker + compose).
...
| Skill | Scripts / scope |
|-------|-----------------|
| [**legacy-on-prem-windows-setup**](../.cursor/skills/legacy-on-prem-windows-setup/SKILL.md) | `Test-OnPremServerPrerequisites.ps1`, `Install-WslDockerEngine.ps1 -SkipDockerInstall`, `.wslconfig` |
...
See [scripts/legacy/on-prem-windows/README.md](../scripts/legacy/on-prem-windows/README.md).

**Skills improve with use:** each on-prem skill keeps **`learnings.md`** (append-only). Agents read it before work and append after verified fixes. Promotion rules: [on-prem-deploy/MATURITY.md](../.cursor/skills/on-prem-deploy/MATURITY.md).

**Scenario tables:** [legacy-on-prem-windows-setup](../.cursor/skills/legacy-on-prem-windows-setup/SKILL.md) · [setup-openssh-server](../.cursor/skills/setup-openssh-server/SKILL.md) · [setup-docker-engine § Scenarios](../.cursor/skills/setup-docker-engine/SKILL.md#scenarios-that-hinder-docker-engine-installation-and-compose)
...
Use this order on **every** new Windows Server. **setup-docker-engine** is **blocked** until **legacy-on-prem-windows-setup** Step 2 passes (`Test-OnPremServerPrerequisites.ps1` **FAIL=0** on WSL/systemd; Docker **WARN** OK).

### Phase 0 — Prerequisites ([legacy-on-prem-windows-setup](../.cursor/skills/legacy-on-prem-windows-setup/SKILL.md))
...
`Install-WindowsOpenSshServer.ps1`, `Repair-WindowsOpenSshServer.ps1` in [scripts/legacy/on-prem-windows/](../scripts/legacy/on-prem-windows/). **Domain-joined:** use `DOMAIN\user` at client; run `Repair-WindowsOpenSshServer.ps1 -TestUser <shortname>` if connection resets.

---

### Phase 2 — WSL optional component (legacy-on-prem-windows-setup)
