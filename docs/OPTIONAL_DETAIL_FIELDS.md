...
| `Visa` | `VisaNumber`, `VisaType`, `VisaCategory`, `VisaIssuedPlace`, dates, `Passport`, `BorderZoneLocation` (default `Ýok`); `IssuingApplicationItem` + `InvitationItem` (read-only, Path A auto-set, `[ExcludeFromOptionalDetailFields]`) | e.g. `ExtensionRequired`, `IsCancelled`, `IsChanged`, `IsExtended`, `Notes` — not `Documents` / `Images` |
