...
| `ApplicationItem` | `Application`, `Person`, document links when `ApplicationType.Show*` (each `[RuleRequiredField]` when visible); when `ShowRegistrations`: **`TravelDate`**, **`CheckPoint`** (required, always visible — `[ExcludeFromOptionalDetailFields]`); **`BorderZoneLocation`** when `ShowBorderZoneLocation` (always visible, not behind gear — `[ExcludeFromOptionalDetailFields]`) | Registration/travel behind gear when `ShowRegistrations`: `RegistrationDate`, `TravelType`, `MovementType`, `TravelNotes`. Purpose of travel = **`CurrentPositionHistory`**. **`CurrentMedicalRecord`**, **`WorkPermittedLocations`** — visible when `Show*` applies but not required on save (no reliable VISA2014 line-level source; import/backfill may leave null/empty/`Ýok`). `[ExcludeFromOptionalDetailFields]` also on `BusinessTripAddress` and workflow status flags. |
