...
| `InvitationItem` | `Invitation`, `Person`, `Passport`, `ExpirationDate` (parent invitation), `IssuedVisa` (always on detail/list) | `InvitationItemName` — cancelled/changed/used are **not** BO properties (`IssuedDocumentLifecycle` in services) |
