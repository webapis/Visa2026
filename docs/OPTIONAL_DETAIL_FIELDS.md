...
| `Visa` | `VisaNumber`, `VisaType`, `VisaCategory`, `VisaIssuedPlace`, dates, `Passport`, `BorderZoneLocation` (default `Ýok`), conditional `IssuingApplicationItem` / `InvitationItem` | e.g. `HasInvitation`, `HistoricalImport`, `ExtensionRequired`, `IsCancelled`, `IsChanged`, `IsExtended`, `Notes` — not `Documents` / `Images` |
