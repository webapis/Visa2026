...
| `Visa` | `VisaNumber`, `VisaType`, `VisaCategory`, `VisaIssuedPlace`, dates, `Passport`, `BorderZoneLocation` (default `Ýok`); `IssuingApplicationItem` + `InvitationItem` (read-only, Path A auto-set, `[ExcludeFromOptionalDetailFields]`) | e.g. `ExtensionRequired`, `IsExtended`, `Notes` — not `Documents` / `Images`. `IsCancelled` / `IsChanged` are derived (not stored, not gear). |
| `Invitation` | `InvitationNumber`, `IssuedDate`, `VisaCategory`, `VisaPeriod`, `ExpirationDate`, `DaysRemaining`, `BorderZoneLocation` (same catalog as Visa, default `Ýok`) | `Application`, `IsVisaStartAndEndDateDefined`, `VisaStartDate`, `VisaEndDate` (visa window) — not `InvitationItems` / `Documents` / `Images` |
| `InvitationItem` | `Invitation`, `Person`, `Passport` (always on detail layout) | `InvitationItemName` — `IsCancelled` / `IsChanged` / `IsUsed` are derived (not stored, not gear) |
...
| `WorkPermitItem` | `Person`, `Passport`, `CurrentPositionHistory`, dates, `WorkPermitNumber`, `ASNumber`, `WorkPermittedLocations`, `DaysRemaining` | Gear no longer includes `IsCancelled` (derived). Change family uses profile **May change** + skip-nav on completed Change instances |
| `ApplicationItem` | `Application`, `Person`, document links when `ApplicationType.Show*` (each `[RuleRequiredField]` when visible); when `ShowRegistrations`: **`TravelDate`**, **`CheckPoint`** (required, always visible — `[ExcludeFromOptionalDetailFields]`); **`BorderZoneLocation`** when `ShowBorderZoneLocation` and **`CurrentMedicalRecord`** when `ShowCurrentMedicalRecord` (always visible, not behind gear — `[ExcludeFromOptionalDetailFields]`) | Registration/travel behind gear when `ShowRegistrations`: `RegistrationDate`, `TravelType`, `MovementType`, `TravelNotes`. Purpose of travel = **`CurrentPositionHistory`**. **`WorkPermittedLocations`** — visible when `Show*` applies but not required on save (no reliable VISA2014 line-level source; import/backfill may leave null/empty/`Ýok`). `[ExcludeFromOptionalDetailFields]` also on `BusinessTripAddress` and workflow status flags. |
