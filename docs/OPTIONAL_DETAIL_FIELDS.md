...
| `Invitation` | `InvitationNumber`, `IssuedDate`, `VisaCategory`, `VisaPeriod`, `ExpirationDate`, `DaysRemaining` | `Application`, `IsVisaStartAndEndDateDefined`, `VisaStartDate`, `VisaEndDate` (visa window) — not `InvitationItems` / `Documents` / `Images` |
