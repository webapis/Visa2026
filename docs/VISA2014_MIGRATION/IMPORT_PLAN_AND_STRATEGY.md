...
**Calik visa+WP extension (locked 2026-09-14):** `App_Visa_and_WP_Ext` / `extend_visa_wp` requires `RequirePersonWorkPermitItem` (the `WorkPermitItem` being extended) plus Visa. People & links hid Work permit while the flag was off (local 5/-1636). After the flag turns on, **re-run** `--correct-visa2014-application-person-document-links`. Produce Work permit is the new issued letter — not a substitute for linking the existing item.
